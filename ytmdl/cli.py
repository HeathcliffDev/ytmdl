...
logger = logging.getLogger("ytmdl")
...
class ProgressBar:
    """Clase para manejar la barra de progreso global"""
    ...
    def _draw(self):
        ...
        percent = int((self.completed_items / self.total_items) * 100) if self.total_items > 0 else 100
        # Asegurar que el porcentaje no pase de 100
        percent = min(percent, 100)
        bar_length = 30
        filled_length = int(bar_length * self.completed_items // self.total_items) if self.total_items > 0 else bar_length
        # Asegurar que filled_length no pase de bar_length
        filled_length = min(filled_length, bar_length)
        bar = '█' * filled_length + '░' * (bar_length - filled_length)
        
        sys.stdout.write(f'\rDescargando: [{bar}] {percent:3d}%')
        sys.stdout.flush()
...
@click.option(
    "--config-path",
    type=Path,
    default=Path.home() / ".ytmdl" / "config.json",
...
def main(
    ...
    # Silenciar logs de info durante la descarga (solo warnings y errores)
    if log_level == "INFO":
    ...
    # Primero, obtener todas las canciones para saber el total
    logger.debug("Scanning URLs...")
    for url in urls:
        try:
            queue = list(downloader.get_download_queue(url))
            download_queue.extend([(url, item) for item in queue])
        except Exception as e:
            error_count += 1
            failed_tracks.append(f"[URL] {url}")
            logger.error(f'Failed to check "{url}"', exc_info=not no_exceptions)
    
    total_tracks = len(download_queue)
    
    if total_tracks == 0 and error_count == 0:
        logger.warning("No tracks found to download")
        return
    
    # Crear barra de progreso
    progress_bar = ProgressBar(total_tracks)
    
    # Descargar cada track
    for url, queue_item in download_queue:
        should_skip = False  # Bandera para saber si nos saltamos esta canción
        ...
        try:
            ...
            if not ytmusic_watch_playlist:
                logger.warning(f'Track "{queue_item["title"]}" doesn\'t have an album or is not available, skipping')
                should_skip = True
                continue
            ...
            if synced_lyrics_only:
                ...
                logger.debug(f'Track already exists at "{final_path}", skipping')
                should_skip = True
                continue
            else:
            ...
            if no_synced_lyrics or not tags.get("lyrics"):
                ...
                logger.debug(f'Synced lyrics already exists at "{synced_lyrics_path}", skipping')
            else:
            ...
            error_count += 1
            failed_tracks.append(queue_item["title"])
            logger.error(f'Failed to download "{queue_item["title"]}"', exc_info=not no_exceptions)
        finally:
            # Solo actualizar el progreso si NO nos saltamos la canción
            if not should_skip:
                progress_bar.update()
            if temp_path.exists():
                downloader.cleanup_temp_path()
    
    # Finalizar y limpiar la línea
    progress_bar.finish()
    ...
    if error_count > 0:
        print("=" * 50)
        print(f"✅ Completado: {total_tracks - error_count} de {total_tracks} canciones")
        print(f"❌ Errores: {error_count}")
        print("-" * 50)
        print("Canciones con error:")
        for failed in failed_tracks:
            print(f"  ✗ {failed}")
        print("=" * 50)
    else:
        print(f"✅ Descarga completada. {total_tracks} canciones descargadas.")